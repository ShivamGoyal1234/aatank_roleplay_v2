...
RegisterNetEvent('esx:playerLoaded', function(id, data)
    ...
    CreateQuests(id)
    local insideLabName = framework:getInside(id)
    if insideLabName then
        TriggerClientEvent('drugs:enterLab', id, insideLabName)
        framework:updateInside(id, nil)
    end
end)
...
function framework:getInside(src)
    ...
    local result = MySQL.prepare.await('SELECT inside_lab FROM users WHERE identifier = ?', { identifier })
