...
Utils.GetForwardVector = function(rot)
    rot = (rot * math.pi / 180.0)
    local z = math.abs(math.cos(rot.x))
    return vec3(-math.sin(rot.z) * z, math.cos(rot.z) * z, math.sin(rot.x))
end

Utils.SplitString = function(str, sep)
...
Utils.BreakString = function(str, len)
    if not str then return '' end
    if #str <= len then return str; end
    local str = string.sub(str, 1, len)
    local lastSpace = string.find(str, ' ', #str - 5)
    if lastSpace then
...
---@param opts { location: vector3, sprite?: number, color?: number, scale?: number, display?: number, shortRange?: boolean, highDetail?: boolean, text: string }
...
Utils.DrawMarker = function(opts)
    ...
    DrawMarker(
        ...
        opts.direction and opts.direction.x or 1.0, opts.direction and opts.direction.y or 0.0,
        opts.direction and opts.direction.z or 0.0,
        opts.rotation and opts.rotation.x or 1.0, opts.rotation and opts.rotation.y or 0.0,
...
Utils.GetControls = function(...)
    ...
    for i = 1, count, 1 do
        ...
        local cameraControl = ActionControls[controlIndex]
        table.insert(res, cameraControl)
    end
    return res
end

Utils.HandleFlyCam = function(cam, boundPos, boundDist, justMouse)
    ...
    if IsDisabledControlPressed(0, controls.up.codes[2]) and not justMouse then
        camPos = camPos + (up * (camOpts.climbSpeed * frameTime))
        didMove = true
    elseif IsDisabledControlPressed(0, controls.up.codes[1]) and not justMouse then
        camPos = camPos - (up * (camOpts.climbSpeed * frameTime))
        didMove = true
    end

    if IsDisabledControlPressed(0, controls.forward.codes[2]) and not justMouse then
        camPos = camPos + (fwd * (camOpts.moveSpeed * frameTime))
        didMove = true
    elseif IsDisabledControlPressed(0, controls.forward.codes[1]) and not justMouse then
        camPos = camPos - (fwd * (camOpts.moveSpeed * frameTime))
        didMove = true
    end

    if IsDisabledControlPressed(0, controls.right.codes[1]) and not justMouse then
        camPos = camPos + (right * (camOpts.moveSpeed * frameTime))
        didMove = true
    elseif IsDisabledControlPressed(0, controls.right.codes[2]) and not justMouse then
        camPos = camPos - (right * (camOpts.moveSpeed * frameTime))
        didMove = true
    end
    ...
    if EnabledMouseMovement then
        ...
        -- if MainCamera then
        --     SetEntityCoords(PlayerPedId(), camPos, false, false, false, false)
...
Utils.ScreenToWorld = function()
    ...
    local rayHandle = StartShapeTestRay(cam3DPos.x, cam3DPos.y, cam3DPos.z, direction.x, direction.y, direction.z, -1, 0,
        4)
...
---@param pos vector3
Utils.CreateObject = function(modelName, pos)
...
Utils.SelectPlayer = function()
    ...
    while true do
        ...
        Utils.DrawMarker({
            ...
            location = GetEntityCoords(peds[index]) +
                vector3(0.0, 0.0, 1.0)
        })
...
string.includes = function(str, comp)
...
-- Add furniture export (insert)
local function AddFurniture(category, items)
    if Config.Furniture[category] then
        table.insert(Config.Furniture[category].items, items)
    else
        print('Error: Category ' .. category .. ' does not exist in furniture settings')
    end
end

exports('AddFurniture', function(category, items)
    return AddFurniture(category, items)
end)
