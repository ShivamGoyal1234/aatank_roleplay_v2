...
const VehicleQueryPage = ({isOpened, vehicles, lang, setVehicleProfileData, isVehicleProfileScreen, theme}) => {
   const [VehiclePlate, SetVehiclePlate] = useState('');
       const [vehicleList, setNewVehicleList] = useState([]);
       ...
       useEffect(() => {
           const prepareData = async () => {
               const search = VehiclePlate.toLowerCase()
        ...
        <div className='vehicle-query-main-frame'>
           <div className={`cquery-head`}>
                <div className={`cquery-put ${theme !== 'dark' ? 'white-bold':''}`}>
                    ...
                    <input placeholder={lang.queryveh} value={VehiclePlate} onChange={(e) => VehiclePlateEvent(e.target.value)}></input>
                </div>
            </div>

            <div className={`cquery-box ${theme !== 'dark' ? 'cqbox':''}`}>
                <div className={`cquery-header ${theme !== 'dark' ? 'white-bold' : ''}`}>
                    <span className={`part-title ${theme !== 'dark' ? 'black-text': ''}`}>{lang.vehicles}</span>
                </div>
                    ...
                    {vehicleList.map((vehicle, index) => (
                        <div className={`citizen-list-item ${theme !== 'dark' ? 'c-list-white' : ''}`}>
                            ...
                            <div style={{width: '5rem',display: 'flex',alignItems: 'center'}}>
                                <div onClick={() => handleSett(vehicle)} className='button-rev2'>
