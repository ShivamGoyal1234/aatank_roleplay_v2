...
const MapApp = ({lang, pName}) => {
    ...
    const [sharedLoctions, setSharedLocs] = useState([]);
    const [blips, setBlips] = useState([]);
    const [pato, setPato] = useState(false);
    const iframeRef = useRef()
    const [selectedLoc, setSelLoc] = useState(null);
    ...
    useEffect(() => {
        const handleMsg = e => {
            if (e.data?.type !== 'blip-click') return;
    ...
    useEffect(()=>{
        if(blips.length||myCoords||sharedLoctions.length){
            ...
            const all=[...blipCoords,...me,...shared]
            setMapUrl(`https://map.0resmon.org/?coords=${all.join('|')}`)
        }
    },[blips,myCoords, sharedLoctions])

    return (
        <div className='map-app-frame'>
            ...
            <div className='share-location-part'>
                <span className='map-app-title'>
                    {lang.citysmap}
                </span>
                <span className='spanlab'>{lang.favourites}</span>
                <div className='fav-loc'>
                    <div onClick={selectLoc} className='fav-loc-item'>
                       <i style={{color: '#fff'}} className="fa-solid fa-plus"></i>
                    </div>
                    <div onClick={selectLoc} className='fav-loc-item'>
                        <img src={`https://docs.fivem.net/blips/radar_jewelry_heist.png`}></img>
                    </div>
                    <div onClick={selectLoc} className='fav-loc-item'>
                        <img src={`https://docs.fivem.net/blips/radar_safehouse.png`}></img>
                    </div>
                    <div onClick={selectLoc} className='fav-loc-item'>
                        <img src={`https://docs.fivem.net/blips/radar_police_station_blue.png`}></img>
                    </div>
                    <div onClick={selectLoc} className='fav-loc-item'>
                        <img src={`https://docs.fivem.net/blips/radar_horde.png`}></img>
                    </div>
                </div>

                {selectedLoc && (<>
                    <span style={{top: '3rem'}} className='spanlab'>{lang.locinfo}</span>
                    <div className='showLocax'>
                        <div onClick={setOnMap} className='markerpoint'><i className="fa-solid fa-car"></i> &nbsp; &nbsp; {selectedLoc}</div>
                    </div>
                </>)}

                <span className='spanlab' style={{top: '3rem', fontSize: '12px'}}>{lang.shmyloca}</span>
                <div onClick={() => setShowModal(true)} className='share-button'><i className="fa-regular fa-share-from-square"></i>{lang.shmyloca}</div>
            </div>

            <div className={`mapapp-modal-overlay ${showModal ? 'mapapp-show' : ''}`}>
                <div className='mapapp-modal-content'>
                    <h3>{lang.whom}</h3>
                    {players.map(pl => (
                        <div key={pl.id} className='mapapp-player-item' onClick={() => shareTo(pl)}>
                            {pl.name}
                        </div>
                    ))}
                    <div className='mapapp-close-modal' onClick={() => setShowModal(false)}>{lang.close}</div>
                </div>
            </div>
