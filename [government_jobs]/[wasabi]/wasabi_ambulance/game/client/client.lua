...
CreateThread(function()
    ...
    if Config.targetSystem then
        ...
        for k, v in pairs(Config.SleepBeds) do
            wsb.target.boxZone('sleepbed_' .. k, v.coords, 2.0, 2.0, {
                heading = v.heading,
                minZ = v.coords.z - 1.0,
                maxZ = v.coords.z + 1.0,
                options = {
                    {
                        event = 'wasabi_ambulance:sleepOnBed',
                        icon = 'fas fa-bed',
                        label = Strings.sleep_on_bed,
                        bedCoords = v.coords,
                        bedHeading = v.heading,
                    }
                }
            })
        end
