...
function DisableRadial(state)
	...
	exports['onex-radialmenu']:disableRadial(state)
end

-- Continuous check to ensure radial menu is disabled when player is dead
CreateThread(function()
	while true do
		Wait(1000) -- Check every second
		if isDead then
			-- Force disable radial menu for dead players
			exports.ox_lib:disableRadial(true)
