...
function Dispatch({ visible, lang, data, index, disableRespond, currentId, location }) {
  ...
  useEffect(() => {
    setIsMounted(true);
    // console.log(data.settingtype, data.type);
  }, []);
  ...
  //       let totalHeight = 0;
  //       const previousElements = document.querySelectorAll(`.${location}.dispatch-alert-box`);
        
  //       for (let i = 0; i < index; i++) {
  //         totalHeight += (previousElements[i]?.offsetHeight || 0) + spacing; 
  //       }
  
  //       setDynamicOffset(totalHeight);
  //     }
  //   }, 50);
  // }, [index, location]);
  ...
  // useEffect(() => {
  //   if (alertBoxRef.current) {
  //     alertBoxRef.current.style.setProperty('top', `${10 + dynamicOffset}px`, 'important');
  //   }
  // }, [dynamicOffset]);
  ...
  >
      <div className='dispatch-alert-box-header'>
          ...
          <span className='dispatch-alert-title'>{!data.custom ? lang[data.type] : data.alertLabel}</span>
          <div className='dispatch-alert-number'>#{data.id}</div>
          <div className={`dispatch-alert-code ${(data.settingtype === 'DeathNotifications' || data.settingtype === 'emsWorkerInjured' || data.settingtype === 'civilianInjured') ? 'alert-code-ems' : ''}`}>{data.code}</div>
          <div className={`dispatch-alert-rightbox ${(data.settingtype === 'DeathNotifications' || data.settingtype === 'emsWorkerInjured' || data.settingtype === 'civilianInjured') ? 'dispatch-alert-box-right-ems' : ''}`}>
            <i className={`fa-solid ${data.icon}`}></i>
          </div>
      </div>
      <div className='dispatch-alert-box-middle'>
        {data.vehicle && data.plateAccess &&   
        <div className='dispatch-alert-box-middle-item'>
            ...
            <div className='dispatch-alert-box-middle-item-right'>
              <p><span className='font-white'>{lang.plate}: </span> {data.eventVehicleData.vehiclePlate}</p>
            </div>
        </div>}
        {data.vehicle &&
          <>
          <div className='dispatch-alert-box-middle-item'>
            ...
            <div className='dispatch-alert-box-middle-item-right'>
              <p>
              <span className='font-white'>{lang.class}: </span> 
              {data.eventVehicleData && typeof data.eventVehicleData.vehicleClassName !== 'undefined' 
                ? data.eventVehicleData.vehicleClassName 
                : lang.unkown}
              </p>
            </div>
          </div>
          <div className='dispatch-alert-box-middle-item'>
            ...
            <div className='dispatch-alert-box-middle-item-right'>
              <p><span className='font-white'>{lang.color}: </span> {data.eventVehicleData.vehicleColorName}</p>
            </div>
          </div>
          </>
        }
        {data.gender && 
          <div className='dispatch-alert-box-middle-item'>
            ...
            <div className='dispatch-alert-box-middle-item-right'>
              <p><span className='font-white'>{lang.gender}: </span> {data.gender}</p>
            </div>
          </div>
        }
        {!data.detail ? <>
          ...
          <div className='dispatch-alert-box-middle-item-right'>
            <p><span className='font-white'>{lang.time}: </span> {lang.justNow}</p>
          </div>
        </div> 
        <div className='dispatch-alert-box-middle-item'>
          <div className='dispatch-alert-box-middle-item-left'>
          ...
          <div className='dispatch-alert-box-middle-item-right'>
            <p><span className='font-white'>{lang.location}: </span> {data.address}</p>
          </div>
        </div></> : null}
        <div className='dispatch-alert-box-middle-item'>
          <div className='dispatch-alert-box-middle-item-left'>
          ...
          {data.witnesses == 0 ? '' : ''}
            <div className='dispatch-alert-box-middle-item-right' style={{width: "54%", float: "left"}}>
              {data.type == 'shooting' ? <p><span className='font-white'>{lang.weapon}: </span> {data.weapon}</p> : <p><span className='font-white'>{lang.responders}: </span> {data.responders}</p>}
            </div>
          
          <div className={`dispatch-alert-box-responde`}>
            {/* <div className='dispatch-alert-box-responde-button'>{data.pressKey}</div> */}
            <span className='dispatch-alert-box-respondo-string'>{lang.respond}</span>
          </div>
        </div>
      </div>
    </div>
