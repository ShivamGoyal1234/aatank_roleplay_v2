...
function ShowFloatingHelpNotification(msg, coords)
    if coords and coords.x and coords.y and coords.z then
        ...
        SetFloatingHelpTextWorldPosition(1, coords.x, coords.y, coords.z) -- Koordinatları ayrı ayrı sağlıyoruz
        SetFloatingHelpTextStyle(1, 1, 2, -1, 3, 0)
        BeginTextCommandDisplayHelp('0rFloatingHelpNotification')
        EndTextCommandDisplayHelp(2, false, false, -1)
    else
        print("ShowFloatingHelpNotification: Invalid coordinates provided.")
    end
end
...
function heistFunc(test)
	currentDialogId = test 
	print("heistFunc: " , test)
end

exports('heistFunc', heistFunc)
...
function closeMenu()
    ...
    if Config.Dialogs[currentDialogId] then
        Config.Dialogs[currentDialogId].Interaction.Text.Enable = false
    end
end
...
function addDialog(dialogData)
    ...
    if type(dialogData) ~= 'table' or not dialogData.Ped or not dialogData.Ped.coords then
        return {success = false, message = 'Invalid dialog data'}
    end

    -- local newDialogId = #Config.Dialogs + 1
	local newDialogId =  math.random(1000, 9999)
	
	TriggerServerEvent('frkn-bankheist:setDialogId', newDialogId)
    
	Config.Dialogs[newDialogId] = dialogData

    -- Ped Oluşturma
    if dialogData.Ped.Enable then
        local pedHash = type(dialogData.Ped.hash) == "number" and dialogData.Ped.hash or joaat(dialogData.Ped.hash)
        RequestModel(pedHash)
        while not HasModelLoaded(pedHash) do
            Citizen.Wait(0)
        end

		dialogData.Ped.ped = dialogData.Ped.ped or 0
        -- FreezeEntityPosition(dialogData.Ped.ped, true)
        -- SetEntityInvincible(dialogData.Ped.ped, true)
        -- SetBlockingOfNonTemporaryEvents(dialogData.Ped.ped, true)
        -- PlaceObjectOnGroundProperly(dialogData.Ped.ped)
        -- SetEntityAsMissionEntity(dialogData.Ped.ped, false, false)
        -- SetPedCanPlayAmbientAnims(dialogData.Ped.ped, false)
        -- SetModelAsNoLongerNeeded(pedHash)
    end

    return {success = true, message = 'Dialog added successfully'}
end
...
function removeDialog(dialogId)
    ...
    if not Config.Dialogs[dialogId] then
		print("Dialog not found")
		return {success = false, message = 'Dialog not found'}
    end

    -- Ped Silme
    if Config.Dialogs[dialogId].Ped.Enable and DoesEntityExist(Config.Dialogs[dialogId].Ped.ped) then
		print("Ped exists")
		DeletePed(Config.Dialogs[dialogId].Ped.ped)
    end
	print("Dialog removed")

    Config.Dialogs[dialogId] = nil

    return {success = true, message = 'Dialog removed successfully'}
end
