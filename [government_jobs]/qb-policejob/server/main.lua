...
RegisterNetEvent('police:server:showFingerprintId', function(sessionId)
    ...
    if Player then
        local citizenid = Player.PlayerData.citizenid
        MySQL.query("SELECT finger FROM players WHERE citizenid = @citizenid", {['@citizenid'] = citizenid}, function(result)
            if result and result[1] and result[1].finger then
                local fid = result[1].finger
                ...
                TriggerClientEvent('QBCore:Notify', src, Lang:t('error.no_fingerprint_found'), 'error')
            end
        end)
    else
