...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    -- ✅ Check if cuffing player is already cuffed
    if Player.PlayerData.metadata["ishandcuffed"] then 
        return TriggerClientEvent('QBCore:Notify', src, "You are cuffed and cannot cuff others", "error")
    end

    -- ✅ Require handcuffs item unless job type is police
    if not Player.Functions.GetItemByName('handcuffs') and Player.PlayerData.job.type ~= 'leo' then 
        return 
    end

    -- 🔓 Proceed with cuffing
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    -- Prevent abuse by checking distance
    -- if #(playerCoords - targetCoords) > 2.5 then 
    --     return DropPlayer(src, 'Attempted exploit abuse') 
    -- end
    ...
    -- No job or state checks — anyone can escort anyone
    TriggerClientEvent('police:client:GetEscorted', EscortPlayer.PlayerData.source, Player.PlayerData.source)
end)
...
RegisterNetEvent('police:server:PutPlayerInVehicle', function(playerId)
    ...
    -- Get target player first
    local EscortPlayer = QBCore.Functions.GetPlayer(playerId)
    if not QBCore.Functions.GetPlayer(src) or not EscortPlayer then return end
    
    -- Check if target player exists before attempting to get coordinates
    local targetPed = GetPlayerPed(playerId)
    ...
    -- Anti-exploit check - only if both players are alive and coordinates are valid
    if playerCoords and targetCoords and #(playerCoords - targetCoords) > 2.5 then 
        return DropPlayer(src, 'Attempted exploit abuse') 
    end

    -- Only allow putting dead or cuffed players in vehicle
    if EscortPlayer.PlayerData.metadata['isdead'] or EscortPlayer.PlayerData.metadata['ishandcuffed'] then
...
RegisterNetEvent('police:server:SetPlayerOutVehicle', function(playerId)
    ...
    -- Check if target player is in a vehicle before attempting to get coordinates
    local targetPed = GetPlayerPed(playerId)
    if not DoesEntityExist(targetPed) then return end
    ...
    -- Only allow taking dead or cuffed players out of vehicle
    if EscortPlayer.PlayerData.metadata['isdead'] or EscortPlayer.PlayerData.metadata['ishandcuffed'] then
