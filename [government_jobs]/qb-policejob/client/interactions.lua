...
RegisterNetEvent('police:client:SetOutVehicle', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if DoesEntityExist(vehicle) then
            -- Force exit vehicle, even if dead
            TaskLeaveVehicle(ped, vehicle, 16)
            -- Additional force exit for dead players
...
RegisterNetEvent('police:client:SearchPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        QBCore.Functions.Notify(Lang and Lang.t and Lang:t('error.none_nearby') or 'No one nearby', 'error')
    end
end)
