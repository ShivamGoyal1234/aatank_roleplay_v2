    ...
    Koci.Server:TriggerCallback("0r-chat:sendMessages", source,
        {
            ...
            text = text,
            sourceId = source,
            channel = "global", -- Changed to global
        })
end)

RegisterCommand("local", function(source, args, rawCommand) -- New local command
    local text = ""
    for _, v in ipairs(args) do
        text = text .. " " .. v
    end
    ...
    Koci.Server:TriggerCallback("0r-chat:sendMessages", source,
        {
            type = "LOCAL",
            color = Config.TypeColors["local"].color, -- Assuming a 'local' type color exists
            borderColor = Config.TypeColors["local"].background, -- Assuming a 'local' type background color exists
            text = text,
            sourceId = source,
            channel = "local",
        })
