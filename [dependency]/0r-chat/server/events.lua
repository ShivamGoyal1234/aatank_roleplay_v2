...
Koci.Server:RegisterServerCallback("0r-chat:sendMessages", function(source, data, cb)
    ...
    if xPlayer then
        ...
        elseif data.channel == "local" then
            ...
            for _, player in ipairs(GetPlayers()) do
                ...
                if distance <= Config.LocalChatRadius then
                    TriggerClientEvent("chat:addMessage", player, {
                        type = data.type,
                        color = data.color,
                        borderColor = data.borderColor,
        ...
        else -- Default to global if channel is not specified or recognized
            for _, player in ipairs(GetPlayers()) do
                TriggerClientEvent("chat:addMessage", player, {
                    type = data.type,
                    color = data.color,
