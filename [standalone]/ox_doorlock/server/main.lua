...
local function isAuthorised(playerId, door, lockpick, jail)
	...
	if player then
		...
		if Config.JobAccess then
			local jobName = player.job and player.job.name or 'unemployed'
			local jobGrade = player.job and player.job.grade or 0
			if Config.JobAccess[jobName] then
				if Config.JobAccess[jobName] >= jobGrade then
					return true
				end
			end
		end
...
---@return boolean
local function setDoorState(id, state, lockpick, jail)
	local door = doors[id]

	state = (state == 1 or state == 0) and state or (state and 1 or 0)

	if door then
		local authorised = not source or source == '' or isAuthorised(source, door, lockpick, jail) or jail
