...
RegisterNetEvent('qb-trunk:client:KidnapTrunk', function()
    ...
    if not isKidnapping or not closestVehicle then
        return exports.qbx_core:Notify(locale("error.not_kidnapped"), 'error')
    end
                ...
                if not isBusy then
                    if not isKidnapped then
                        if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                            ...
                            lib.playAnim(cache.ped, "fin_ext_p1-7", "cs_devin_dual-7", 8.0, 8.0, -1, 1, 999.0, false,
                                false, false)
                            AttachEntityToEntity(cache.ped, closestVehicle, 0, offset.x, offset.y, offset.z, 0, 0, 40.0,
