...
function takeHostage()
	...
	if closestPlayer ~= nil and closestPlayer ~= -1 then
		...
		if NoWeapon then
			TriggerClientEvent('QBCore:Notify', src, 'You cannot use this weapon to take a hostage!', 'error')
			return
		end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"[G] Release, [H] Kill")
