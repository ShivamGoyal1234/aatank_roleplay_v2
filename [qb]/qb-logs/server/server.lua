...
AddEventHandler('qb-log:server:CreateLog', function(name, title, color, message, tagEveryone)
    ...
    PerformHttpRequest(webHook, function(err, text, headers) end, 'POST', json.encode({ username = "Aatank Logs",embeds = embedData}), { ['Content-Type'] = 'application/json' })
    Citizen.Wait(100)
    if tag then
        PerformHttpRequest(webHook, function(err, text, headers) end, 'POST', json.encode({ username = "Aatank Logs", content = "@everyone"}), { ['Content-Type'] = 'application/json' })
    end
end)
