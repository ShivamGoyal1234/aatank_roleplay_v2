...
registerCommand('h2hsetup', 'Setup Impromptu',{}, false, function(source)
    local racerName = exports['cw-racingapp']:getActiveRacerName(source)
    if not racerName then
        TriggerClientEvent('cw-racingapp:client:notify', source, Lang('error.lacking_user'), 'error')
        return
    end
    local auth = exports['cw-racingapp']:getUserAuth(racerName)
    if not auth or not auth.setup then
        TriggerClientEvent('cw-racingapp:client:notify', source, Lang('error.no_permissions'), 'error')
        return
    end
